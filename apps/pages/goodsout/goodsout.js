mobileSJD.config(["$stateProvider", "base", function($stateProvider, base) {
    $stateProvider.state("goodsout", {
        url: "/goodsout",
        templateUrl: base + "/pages/goodsout/goodsout.html",
        controller: "goodsoutCtrl"
    });
}])

.controller("goodsoutCtrl", ["$scope", "$state", "$uibModal", "$http", "xhrRequestOrigin", "base", "sjdDialog", "Constants",
    function($scope, $state, $uibModal, $http, xhrRequestOrigin, base, sjdDialog, Constants) {
        ...
        $scope.addGoods = function() {
            $uibModal.open({
                templateUrl: base + "/pages/goodsout/add.html",
