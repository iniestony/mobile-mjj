mobileSJD.config(["$stateProvider", "base", function($stateProvider, base) {
        $stateProvider.state("application", {
            url: "/application",
            templateUrl: base + "/pages/application/application.html",
            controller: "applicationCtrl"
        });
    }])
    .controller("applicationCtrl", ["$scope", "$rootScope", "$state", "$uibModal", "$http", "xhrRequestOrigin", "base",
        function($scope, $rootScope, $state, $uibModal, $http, xhrRequestOrigin, base) {
            ...
            var getPictures = function() {
                $scope.pictures = {};
                $scope.keys = [];
                $http.get(xhrRequestOrigin + "/loanapplicationdoc/app/loandocs/show.do?customerprojectid=" + $rootScope.project.id)
            ...
            $http.get(xhrRequestOrigin + "/loanapplication/app/form/getform.do?customerprojectid=" + $rootScope.project.id)
                .success(function(data) {
            ...
            $scope.upload = function(item) {
                $uibModal.open({
                    templateUrl: base + "/pages/application/upload.html",
            ...
            if ($rootScope.project.intention != undefined) {
                $scope.amount = $rootScope.project.intention.amount / 10000;
                $scope.duration = $rootScope.project.intention.duration;
                $scope.usage = $rootScope.project.intention.usage;
            } else {
                $scope.amount = 0;
            }
            ...
            $scope.step1to2 = function() {
                ...
                $http({
                    ...
                    $rootScope.project.intention.amount = $scope.amount * 10000;
                    $rootScope.project.intention.duration = $scope.duration;
                    $rootScope.project.intention.usage = $scope.usage;
                });
                $scope.step = 2;
            }
    ...
    .controller("uploadCtrl", ["$scope", "$rootScope", "$uibModalInstance", "$http", "xhrRequestOrigin", "item", "images", "imageReader",
        function($scope, $rootScope, $uibModalInstance, $http, xhrRequestOrigin, item, images, imageReader) {
            ...
            $scope.uploadImage = function() {
                document.getElementById("image-commit").click();
            };
